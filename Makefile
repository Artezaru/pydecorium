...
latexpdf:
	...
	cd $(BUILDDIR)/latex && pdflatex pydecorium.tex && pdflatex pydecorium.tex
...
app:
	echo "from pydecorium.__main__ import __main_gui__" > run_gui.py
	echo "__main_gui__()" >> run_gui.py
	pyinstaller --name pydecorium --onefile --windowed run_gui.py
	rm run_gui.py
	rm -rf build
	rm run_gui.spec
